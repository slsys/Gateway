...
### Вариант 1 с использованием однократного таймера
...
```
### Вариант 2 с использованием таймера Cron

Предыдущий вариант управляет только одним выключателем, т.е. под каждый выключатель надо будет создавать отдельный скрипт (даже по два), чтобы они не конкурировали друг с другом. Начиная с прошивки 2024.01.22d4, появилась возможность задавать несколько таймеров для одного скрипта. Пример использования этой возможности для автоматического отключения света через заданный промежуток времени описан [в отдельной статье](/lua_doc/LightTimeOut.md).
