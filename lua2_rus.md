...
При запуске системы единоразово выполняется `init.lua`. Перед началом работы со скриптами, рекомендуется проверить его наличие рядом со всеми остальными скриптами `*.lua`. Если файла нет, то его нужно создать так же как и вышеописанный `test.lua`.  В нем полезно инициализировать переменные для работы с устройством, а также выполнить какие либо разовые действия. Например:
```lua
-- init.lua --
...
1. [при изменении состояния устройства](lua2_rus.md#запуск-скрипта-при-изменении-состояния-устройства)
2. [Запуск скрипта по изменению объекта](lua2_rus.md#выполнение-сценария-по-изменению-объекта)
3. [Запуск lua скрипта с помощью  http api](lua2_rus.md#запуск-скрипта-с-помощью--http-api)
4. [Запуск lua скрипта из другого скрипта](lua2_rus.md#запуск-lua-скрипта-из-другого-скрипта)
5. [Периодическй запуск lua скриптов (таймеры)](lua2_rus.md#периодический-запуск-скриптов-таймеры)
6. [Скрипт инициализации](lua2_rus.md#скрипт-инициализации)
7. Запуск lua скрипта по подписке mqtt. В разработке.

### Запуск скрипта при изменении состояния устройства
На вкладке Zigbee необходимо зайти в параметры устройства и в окне SimpleBind указать имя файла скрипта (префикс "/" необязателен) 
![](/img/execlua.jpg)
