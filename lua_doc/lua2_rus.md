...
1. [при изменении состояния устройства](/lua_doc/lua2_rus.md#запуск-скрипта-при-изменении-состояния-устройства)
2. [по событию изменения объекта](lua2_rus.md#запуск-скрипта-по-событию-изменения-объекта) <!--	#ToDo - в документации вендора данный пункт ссылается на несуществующий раздел? -->
3. [запуск из другого скрипта](lua2_rus.md#запуск-lua-скрипта-из-другого-скрипта)
4. [с помощью  http api](lua2_rus.md#запуск-скрипта-с-помощью-http-api)
5. [периодический запуск (планировщик)](lua2_rus.md#периодический-запуск-скриптов-таймеры)
6. по подписке mqtt (в разработке).
