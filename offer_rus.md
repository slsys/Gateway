...
| Q2 2020 | [Доработаны некоторые конвертеры Xiaomi для команды get. Скрипты: Переименована функция GetState() в GetStateValue(). Добавлена ф-я GetState() для отправки get команды в конвертер. Функции GetState() /SetState() теперь возвращают true / false от результата. Добавлена ф-я GetUnixTime() для получения Unix времени, ф-я GetObj / SetObj() для сохранения и получения объекта для обмена данными между скриптами, ф-я MQTTPub() для отправки произвольных данных в MQTT.](https://t.me/slssys/57)| Реализовано |
